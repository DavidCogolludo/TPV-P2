...
bool loadTexture(SDL_Renderer* pRenderer, SDL_Texture* & pTexture, string const s);  // crea una textura a partir de un archivo y define la variable pTexture 
...
const int SCREEN_WIDTH = 400;   //Screen dimension
const int SCREEN_HEIGHT = 400;  //Screen dimension
...
int main(int argc, char* args[]) {  // SDL require esta cabecera 
	...
	loadTexture(pRender, pTexture, "ini");
	...
	if (pWin == nullptr || pRender == nullptr || pTexture == nullptr) {
		...
		exit = handle_events();	    //  
		while (exit == 1) {    // While application is running
			update();
			//SDL_Delay(2000);  //Wait two seconds
			render(pRender, pTexture);
			exit = handle_events();
		}
		if (exit == -1) jugar = false;
	}
...
bool loadTexture(SDL_Renderer * pRenderer, SDL_Texture* & pTexture, string const s) {
	//The image we will load 
	std::string bmpName{ "..\\bmps\\"+s+".png"};

	SDL_Surface* pTempSurface = nullptr;
	bool success = true;  //Loading success flag

	//Load image
	pTempSurface = IMG_Load(bmpName.c_str());  // si usamos string: bmpName.c_str() 
	if (pTempSurface == nullptr) {
		cout << "Unable to load image " << bmpName.c_str() << "! \nSDL Error: " << SDL_GetError() << '\n';
		success = false;
	}
	else {
